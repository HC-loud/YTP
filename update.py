...
import json
import os
import re  # 파일명에서 버전 숫자만 뽑아내기 위한 모듈 추가
...
def update_json():
    ...
    # 🔥 핵심: 파일명이나 릴리즈 제목에서 진짜 앱 버전(예: 19.12.3) 추출하기
    # 정규표현식으로 숫자.숫자.숫자 형태를 찾습니다.
    version_match = re.search(r'(\d+\.\d+(?:\.\d+)?)', filename)
    if not version_match:
        # 파일명에 없으면 릴리즈 제목에서 한 번 더 찾습니다.
        version_match = re.search(r'(\d+\.\d+(?:\.\d+)?)', data['name'])
    
    # 찾은 진짜 버전을 저장 (못 찾으면 기존처럼 태그명 사용)
